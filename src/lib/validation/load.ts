...
import { categorySchema, complianceSchema, funnelSchema, marketingSchema, productSchema, seoSchema, socialProfileSchema, type Category, type Compliance, type Funnel, type Marketing, type Product, type Seo } from '../../schemas/manifests';
...
export type ProductRecord = { product: Product; funnel: Funnel; compliance: Compliance; seo: Seo; directory: string };
export function loadProducts(): ProductRecord[] {
  ...
  return fs.readdirSync(base, { withFileTypes: true }).filter(x => x.isDirectory()).map(entry => {
    ...
    return { directory, product: productSchema.parse(read(path.join(directory, 'product.yaml'))), funnel: funnelSchema.parse(read(path.join(directory, 'funnel.yaml'))), compliance: complianceSchema.parse(read(path.join(directory, 'compliance.yaml'))), seo: seoSchema.parse(read(path.join(directory, 'seo.yaml'))) };
  });
}
