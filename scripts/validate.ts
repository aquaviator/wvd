...
try {
  ...
  for (const record of products) {
    ...
    if (compliance.regulatory_sensitive && product.status === 'active') {
      if (!compliance.verified_at || !compliance.review_by || !compliance.disclaimer) errors.push(`${product.product_id}: active regulatory product lacks mandatory review information`);
